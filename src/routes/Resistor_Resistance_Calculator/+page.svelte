<script>
	...
	function calculate_resistance() {
		let band1_new = parseInt(band1);
		let band2_new = parseInt(band2);
		let band3_new = parseInt(band3);
		let band4_new = parseInt(band4);
		let resistance = (band1_new * 10 + band2_new) * Math.pow(10, band3_new);
		if (band4) {
			let tolerance_value = resistance * (band4_new / 100);
			let upper_limit = resistance + tolerance_value;
			let lower_limit = resistance - tolerance_value;
			resistanceResult = lower_limit.toString() + ' ohms - ' + upper_limit.toString() + ' ohms';
		} else {
